...
import pytest
from ampf.auth import TokenExp, Tokens
from ampf.base import BaseFactory
from ampf.testing import ApiTestClient
from core.users.user_model import User, UserHeader


# Extra fixture with cleaning users in database because these tests count users
@pytest.fixture(scope="function")
def tokens(factory: BaseFactory, client: ApiTestClient) -> Generator[Tokens]:
    # Login
    yield client.post_typed("/api/login", 200, Tokens, data={"username": "test", "password": "test"})
    # Clean database
    factory.create_compact_storage("token_black_list", TokenExp, "token").drop()
    factory.get_collection("users").drop()
...
def test_get_all_unauthenticated(client: ApiTestClient):
    # When: I get all users without authentication
    # Then: I get 401
