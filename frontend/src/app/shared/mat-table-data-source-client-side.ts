...
/**
 * Źródło danych dla tabeli z filtrowaniem, paginacją
    ...
    */
    ...
    override connect(): BehaviorSubject<ItemType[]> {
        ...
        if (this.paginator && this.sort)
            this.setPaginatorAndSort(undefined, undefined)
        return super.connect();
    }

    /**
     * Ustawia obiekty paginator i sort oraz je subskrybuje w
     * celu przechwycenia zmian i ustawienia odpowiednio URL
     * @param paginator 
     * @param sort 
     */
    public setPaginatorAndSort(paginator: MatPaginator | undefined, sort: MatSort | undefined) {
        if (paginator)
            this.paginator = paginator
        if (sort)
            this.sort = sort
        if (this.paginator && this.sort) {
            merge(this.filterChange, this.paginator.page, this.sort.sortChange).subscribe(() => {
                this.updateUrl(this.createParams());
            });
        }
    }
    ...
    public getData(): Observable<HttpResponse<ItemType[]>> {
        if (this.http && this.apiUrl)
            ...
            throw new Error('Dla domyślnego pobierania danych trzeba podać http i apiUrl.');
    }
     ...
     * odpowiednio ustawiając obiekty filtrujące,
    ...
    /**
     * Odświeża dane w tabeli, pobierając je ponownie z serwera.
     */
    public refresh() {
        this.connect();
    }
